...
class NeuMF_my_cikm_p():
    def __init__(self, sess, num_user, num_item, learning_rate=0.5, reg_rate=0.01, epoch=9, batch_size=25,
                 verbose=True, T=1, display_step=1000,A2C_weight=100,center_weight=0.001,pseudo_weight=0.001):
...
#####################################################################
        ...
        self.pred_long = tf.nn.sigmoid(
            avg_GMF+tf.reduce_sum(5 * tf.multiply(user_feature, target_item_feature), 1))
        # self.pred_y = tf.layers.dense(inputs=tf.concat([GMF, MLP], axis=1), units=1, activation=tf.sigmoid, kernel_initializer=tf.random_normal_initializer, kernel_regularizer= tf.contrib.layers.l2_regularizer(scale=self.reg_rate))
...
##################################################
    ...
    def train(self):
        ...
        ################################################
        # param = 2 / (1 + np.exp(-10 * (self.epoch) / 1000)) - 1
        param = 2 / (1 + np.exp(10 * (self.epoch+2)))
        # print(param)
        # self.sess.run(tf.assign(self.par, param))
        ###############################################
        # train
        ...
        for i in range(self.total_batch):
            ...
            _, loss, total_loss,pred, DAloss,pseudo_loss = self.sess.run((self.optimizer, self.loss,self.total_loss,self.pred_y,self.DAloss,self.pseudo_loss),
                                    feed_dict={self.user_id: batch_users, self.item_id: batch_items, self.y: batch_labels,self.target_item_id:batch_targets, self.hot_item_id:batch_hot_items,self.long_item_id:batch_long_items,self.par:param})
